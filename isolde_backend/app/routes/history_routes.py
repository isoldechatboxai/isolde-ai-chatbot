...
def get_history():
    ...
    try:
        ...
        current_app.logger.error(
            f"Error fetching history for user {user_id}: {e}"
        )

        return jsonify({
            "status": "error",
            "message": (
                "Failed to load chat history due to "
                "a server error."
            ),
        }), 500
...
def get_conversation(conversation_id):
    ...
    try:
        ...
        return jsonify({
            "status": "error",
            "message": (
                "Failed to load conversation due to "
                "a server error."
            ),
        }), 500
...
def update_conversation(conversation_id):
    ...
    try:
        ...
        return jsonify({
            "status": "error",
            "message": (
                "Failed to update conversation due to "
                "a server error."
            ),
        }), 500
    ...
    try:
        ...
        if "is_pinned" in data:
            conversation.is_pinned = _to_bool(
                data.get("is_pinned")
            )

        if "is_archived" in data:
            conversation.is_archived = _to_bool(
...
def delete_conversation(conversation_id):
    ...
    try:
        ...
        return jsonify({
            "status": "error",
            "message": (
                "Failed to delete conversation due to "
                "a server error."
            ),
        }), 500
...
def delete_all_history():
    ...
    try:
        ...
        current_app.logger.error(
            f"Error deleting all history for user "
            f"{user_id}: {e}"
        )
        ...
        return jsonify({
            "status": "error",
            "message": (
                "Failed to clear history due to "
                "a server error."
            ),
        }), 500
...
def export_conversation(conversation_id):
    ...
    try:
        ...
        return jsonify({
            "status": "error",
            "message": (
                "Failed to export conversation due to "
                "a server error."
            ),
        }), 500
    ...
    response.headers["Content-Disposition"] = (
        f'attachment; '
        f'filename="conversation-{conversation.id}.json"'
    )
