...
def register():
    ...
    # In production: send a real verification email instead of returning the token
    return jsonify({
...
def login():
    ...
    log_event(current_app, "LOGIN_SUCCESS", f"{email}", user.id)
...
def forgot_password():
    ...
    if not user:
        ...
        return jsonify({"message": "If that account exists, an OTP has been sent."})
    ...
    # In production: email the OTP instead of returning it directly
    return jsonify({
        "message": "If that account exists, an OTP has been sent.",
        "dev_otp": otp,  # remove this field in production
    })
...
def reset_password():
    ...
    if not user.reset_otp_expires or datetime.now(timezone.utc) > user.reset_otp_expires.replace(tzinfo=timezone.utc):
